        ...
        <div className="flex flex-col gap-2 w-full :max-w-[250px]">
            ...
            <div className="border border-gray-300 rounded-xs">
                <div className="relative">
                    <Image src={imagem} alt={nome} width={233} height={290} loading="eager" />
                    <div className="absolute bg-gray-100 p-1 top-0 right-0">
                        <span className="text-blue-400 font-bold" aria-label="Produto exclusivo">
                            EXCLUSIVO!
                        </span>
                    </div>
                    <div className="absolute bg-background border border-gray-300 border-l-0 rounded-xs rounded-tr-xl p-2 pl-20 bottom-0 left-0 flex items-center">
                        <div className="absolute -top-5 left-0 ">
                            <PackageOpen
                                color="#7ccf00"
                                className="bg-white absolute w-18 h-18 inline-block mr-1"
                                aria-hidden="true"
                            />
                        </div>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="grid grid-cols-6 gap-2 w-fit">
                            {cores.map((cor) => (
                                <button
                                    key={cor}
                                    className={`${cor} w-4 h-4 rounded-full cursor-pointer select-none`}
            ...
            {onClick && (
                <button
                    onClick={onClick}
                    className="bg-lime-500 w-full uppercase text-white font-bold py-1 rounded-sm cursor-pointer hover:opacity-75"
                    aria-label={`Conferir produto ${nome}`}
                >
                    Confira
                </button>
            )}
